...
using KahaGameCore.Package.GameFlowSystem.Samples;
...
namespace KahaGameCore.Package.GameFlowSystem.Samples.Editor
{
    /// <summary>
    /// 範例 UI 一鍵建置工具：產生所有 Sample View prefabs（Samples/Resources/SampleUIViews）、
    /// 整合用的 GameFlowSampleRoot.prefab（拖進任意場景即可運行；unpack 後即可自由修改），
    /// 以及示範場景 GameFlowSampleScene.unity。版面以 3840x2160（4K）為設計解析度。
    /// 可重複執行（會覆寫既有產物）。
    /// </summary>
    public static class SampleUiBuilder
    {
        ...
        [MenuItem("KahaGameCore/GameFlowSystem/Build Sample UI Prefabs And Scene")]
        public static void BuildAll()
        {
            ...
            BuildRootPrefabAndScene();

            AssetDatabase.SaveAssets();
            Debug.Log($"[SampleUiBuilder] Sample UI prefabs、{ROOT_PREFAB_PATH} 與示範場景建置完成（3840x2160）。");
        }
        ...
        /// <summary>
        /// 產生 GameFlowSampleRoot.prefab（Camera + EventSystem + Canvas + DialogueView + Launcher
        /// 全部接好，拖進任意場景即可運行；unpack 後即可自由修改），並以它建立示範場景。
        /// </summary>
        private static void BuildRootPrefabAndScene()
        {
            Scene scene = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);

            GameObject root = new GameObject("GameFlowSampleRoot");
            ...
            GameObject launcherObject = new GameObject("SampleGameLauncher");
            launcherObject.transform.SetParent(root.transform, false);
            SampleGameLauncher launcher = launcherObject.AddComponent<SampleGameLauncher>();
            ...
            GameObject rootPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(ROOT_PREFAB_PATH);
        ...
        private static GameObject InstantiateDialogueView(Transform canvasTransform)
        {
            ...
            if (prefab == null)
            {
                Debug.LogError($"[SampleUiBuilder] 找不到 DialogueView prefab：{DIALOGUE_VIEW_PREFAB_PATH}，請手動拖入場景並指定給 SampleGameLauncher。");
                return null;
            }
