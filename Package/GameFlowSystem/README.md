...
- **介面繼承**：`ITimeService : IGameFlowTimeService`（以 `new TimePhaseData CurrentPhase` 覆蓋成具體表格型別供 HUD 使用，實作類別再顯式實作 `IGameFlowTimeService.CurrentPhase`）；`IGameState : IGameFlowState`、`ICommandExecutor : IGameFlowCommandExecutor` 等則是直接繼承、刪掉重複宣告。
- **資料類別**：`TimePhaseData : IGameData, IGameFlowTimePhase`（`bool IGameFlowTimePhase.AllowAction => AllowAction == 1;`）、`PlayerActionData : IGameData, IGameFlowAction`（欄位名稱相同，零成本）。
- **Provider**：`PlayerActionProvider` 顯式實作 `IGameFlowActionProvider`，利用 `IReadOnlyList<T>` 協變直接回傳原清單。
- **組裝根**：`Assets/ProjectII/Scripts/GameLauncher.cs` 的 `EnsureServicesBuilt()` 與 `OnReturnToTitleRequested()`。
