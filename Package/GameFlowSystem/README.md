...
UI 層（`IActionMenuPresenter`、`IHintPresenter`、`ILocationMenuPresenter`）與演出（`IStagePerformance`）刻意不提供預設——它們綁定各專案的美術資產與設計解析度，請參照 ProjectII 的 Presentation 層實作（純轉接，每個約 30 行）。
...
- **組裝根與 UI 層**：`Assets/ProjectII/Scripts/GameLauncher.cs`（Builder 用法、返回標題的 CancelFlow + CancelPending）與 `Assets/ProjectII/Scripts/Presentation/`（三個 Presenter 的轉接寫法）。
