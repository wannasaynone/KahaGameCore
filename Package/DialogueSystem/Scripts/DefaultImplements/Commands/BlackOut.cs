...
{
    public class BlackOut : DialogueCommandBase
    {
        public override void Process(string[] args, DialogueContext context)
        {
            ...
            if (args.Length > 0 && !string.IsNullOrEmpty(args[0]))
            {
                ...
                {
                    Debug.LogError($"Invalid fadeTime value in BlackOut command: {args[0]}");
                }
            }
        ...
        private async UniTask ProcessAsync(DialogueContext context, float fadeTime, bool waitForCompletion)
        {
            try
            {
                ...
                {
                    await context.view.BlackOut(fadeTime);
                    context.onComplete?.Invoke();
                }
                else
                {
                    // Start the effect but don't wait for it to complete
                    _ = context.view.BlackOut(fadeTime);
                    context.onComplete?.Invoke();
                }
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError($"Error in BlackOut command: {ex.Message}");
                context.onComplete?.Invoke();
            }
        }
    }

    public class BlackOutFactory : DialogueCommandFactoryBase
    {
